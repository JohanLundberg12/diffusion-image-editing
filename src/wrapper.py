...
def make_vid_from_pil_imgs(
    imgs, tmp_folder="vid_tmp/", framerate=4, out_name="out.mp4"
):
    import shutil
...
class StableDifussionWrapper:
                ...
                if i >= start_step:
                    ...
                    if self.scheduler_type == "lms":
                        ...
                        if return_latents_t0_preds:
                            latents_t0_pred = (
                                latents.detach().clone() - sigma * noise_pred
                            )
    ...
    def z_to_pil(self, z0, size=(512, 512)):
        ...
        if n == 1:
            ...
            return [
                tensor_to_pil(self.decode(z.unsqueeze(0)))[0].resize(size) for z in z0
            ]
    ...
    def interpolation(self, z1, z2, numsteps=5, t_min=0, t_max=1, method="slerr"):
        return torch.cat(
            [self.lerp(z1, z2, t) for t in torch.linspace(t_min, t_max, numsteps)]
        )
